...
describe("papers-meta", () => {
  it("has twenty-three implemented papers in MVP", () => {
    const impl = PAPERS_META.filter((p) => p.status === "implemented").map((p) => p.slug);
    expect(impl.sort()).toEqual(["adam", "alexnet", "bahdanau", "batchnorm", "bert", "cfg", "chinchilla", "clip", "ddpm", "dropout", "flashattention", "flow-matching", "gan", "gpt2", "gpt3", "gqa", "ldm", "mamba", "react", "resnet", "seq2seq", "transformer", "vit"]);
  });
