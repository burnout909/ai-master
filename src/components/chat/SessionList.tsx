  ...
  return (
    <div className="flex flex-col h-full">
      <button
        ...
        className="mx-4 my-3 text-[13px] text-left border border-line rounded-[4px] px-3 py-2 hover:border-ink"
        aria-label="새 대화 시작"
      >＋ 새 대화</button>
      <ol className="flex-1 overflow-y-auto px-4 pb-4 flex flex-col" role="list">
        {sorted.map((s) => (
          <li key={s.id} className="border-b border-line/60 py-2.5">
            <div className="flex items-center gap-2.5">
              ...
              <button
                ...
                className="text-[12px] text-mute hover:text-ink px-1" title="이름 바꾸기"
                aria-label="세션 이름 바꾸기"
              >✎</button>
              <button
                onClick={() => { if (confirm("이 세션을 삭제할까?")) store.deleteSession(s.id); }}
                className="text-[12px] text-mute hover:text-[color:var(--danger)] px-1" title="삭제"
                aria-label="세션 삭제"
              >×</button>
            </div>
          </li>
